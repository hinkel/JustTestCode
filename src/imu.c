...
bool    newbaroalt = false, GroundAltInitialized = false, HaveNewMag = false;
...
static void getEstimatedAttitude(void)
{
    ...
    static float    accLPFALT[3], accLPFGPS[3], Tilt_25deg, AccScaleCMSS;
    static float    INV_GYR_CMPF_FACTOR, INV_GYR_CMPFM_FACTOR, ACC_GPS_RC, ACC_ALT_RC, ACC_RC;
    static uint32_t UpsDwnTimer;
    static bool     init = false;
    float           tmp[3], scale, deltaGyroAngle[3], ACCALTFac, ACCGPSFac, ACCFac, rollRAD, pitchRAD;
    float           NormFst = 0.0f, NormScnd, NormR, A, B, cr, sr, cp, sp, cy, sy, spcy, spsy, acc_south, acc_west, acc_up;
    uint8_t         i;
    uint32_t        tmpu32;
    if(!init)                                                                 // Setup variables & constants
    {
        init = true;
        AccScaleCMSS         = 980.665f / cfg.sens_1G;                        // scale to cm/ss
        ...
        {
            tmp[0]       = accADC[i] * AccScaleCMSS;
            accSmooth[i] = tmp[0];
            accLPFGPS[i] = tmp[0];
            accLPFALT[i] = tmp[0];
            EstG.A[i]    = tmp[0] * 0.5f;
            EstM.A[i]    = magADCfloat[i];
        }
    }
    ...
    for (i = 0; i < 3; i++)
    {
        tmp[0]            = accADC[i]    *  AccScaleCMSS;                     // Scale to CM/ss from here on
    ...
    else NormR = 1.0f;                                                        // Vanilla value for "should never happen case" to keep things going
    for (i = 0; i < 3; i++) tmp[i] = EstG.A[i] * NormR;                       // tmp[0..2] contains normalized EstG
    tmpu32   = (uint32_t)(NormFst * 1.0197266f);                              // For comparison Scale to 1G*1000 (NormFst * 1000/980,655)
    if (600 < tmpu32 && tmpu32 < 1400)                                        // Gyro drift correction if ACC between 0.6G and 1.4G (mwii 2.1 scale)// mwii 2.2 Scale 0.85G - 1.15G (850 < tmpu32 && tmpu32 < 1150)
    {
    ...
    else if(!UpsDwnTimer) UpsDwnTimer = currentTime + 20000;                  // Use Timer here to make absolutely sure we are upsidedown
    ...
    {                                                                         // Note: if no mag or not calibrated hdg = 0 Acc done in bodyframe later
        ...
        A       = tmp[1] * cp + tmp[0] * sr * sp + tmp[2] * cr * sp;          // Note: One cycle after successful cmpf is done with old values.
        B       = tmp[0] * cr - tmp[2] * sr;
    ...
    spcy      = sp * cy;
    spsy      = sp * sy;
    acc_up    = ((-sp) * accLPFALT[1] + sr * cp * accLPFALT[0] + cp * cr * accLPFALT[2]) - 980.665f;
    if(GroundAltInitialized) vario += acc_up * ACCDeltaTimeINS * constrain(TiltValue + 0.05, 0.5f, 1.0f);// Positive when moving Up. Just do Vario when Baro completely initialized. Empirical hightdrop reduction on tilt.
    ...
    ACC_speed[LAT] -= acc_south * ACCDeltaTimeINS;                            // Positive when moving North cm/sec when no MAG this is speed to the front
    ACC_speed[LON] -= acc_west  * ACCDeltaTimeINS;                            // Positive when moving East cm/sec when no MAG this is speed to the right
}
...
void getAltitudePID(void)                                                     // I put this out of getEstimatedAltitude seems logical
{
    ...
    BaroI    = (int16_t)(((float)cfg.I8[PIDALT] * vario * INVACCDeltaTimeINS) * 0.00006f); // That is actually a "D"
